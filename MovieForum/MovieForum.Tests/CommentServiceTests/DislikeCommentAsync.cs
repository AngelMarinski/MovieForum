...
{
    [TestClass]
    public class DislikeCommentAsync
    {
        private static IMapper _mapper;
        private MovieForumContext context;

        public DislikeCommentAsync()
        {
            if (_mapper == null)
            {
        ...
        [TestMethod]
        public async Task Should_DislikeComment_When_UserDidNotEverReactedToThisComment()
        {
            await context.AddRangeAsync(Helper.Comments);
            await context.AddRangeAsync(Helper.Users);
            await context.AddRangeAsync(Helper.Movies);
            ...
            Assert.AreEqual(1, firstComment.DisLikesCount);
        }

        [TestMethod]
        public async Task Should_RemoveDislike_When_UserAlreadyDislikedComment()
        {
            await context.AddRangeAsync(Helper.Comments);
            await context.AddRangeAsync(Helper.Users);
            await context.AddRangeAsync(Helper.Movies);
        ...
        [TestMethod]
        public async Task Should_RemoveLikeAndAddLike_When_UserAlreadyLikedComment()
        {
            await context.AddRangeAsync(Helper.Comments);
            await context.AddRangeAsync(Helper.Users);
            await context.AddRangeAsync(Helper.Movies);
